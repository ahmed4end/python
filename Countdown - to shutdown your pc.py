...
class window(QWidget):
	...
	def ui(self):
		...
		#checkboxes column 2.
		self.checkbox_2h = QCheckBox('2 houres')
		self.checkbox_3h = QCheckBox('3 houres')
		self.checkbox_4h = QCheckBox('4 houres')
		self.checkbox_5h = QCheckBox('5 houres')
	...
	def btnClicked(self , button):
		try:
			if button.text() == 'Shutdown':
				...
				self.label.setText(f'Pc will shutdown in {str(int(self.lineEdit.text()) / 60)} h.')
			...
			warning = QMessageBox.question(self, 'Warning', "Please, Enter only numbers (minutes).?!", QMessageBox.Ok | QMessageBox.No, QMessageBox.No)
